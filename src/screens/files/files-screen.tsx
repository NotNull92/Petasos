...
function Breadcrumb({ path }: { path: string }) {
  ...
  return (
    <div className="flex items-center gap-1 truncate text-xs text-primary-500 dark:text-neutral-400 min-w-0">
      <span className="shrink-0">워크스페이스</span>
      {parts.map((part, i) => (
  ...
  const footer = (
    <div className="flex shrink-0 items-center gap-4 border-t border-primary-200 dark:border-neutral-800 px-4 py-1.5 text-xs text-primary-400 dark:text-neutral-500">
      ...
      {selectedEntry.modifiedAt && (
        <span>수정됨 {formatDate(selectedEntry.modifiedAt)}</span>
      )}
      {dirty && (
        <span className="text-accent-500 font-medium">저장하지 않은 변경사항</span>
      )}
    </div>
  )
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex flex-1 min-h-0 items-center justify-center overflow-auto p-6">
            {dataUrl ? (
              ...
              <div className="text-sm text-primary-400">미리보기 없음</div>
            )}
          </div>
          {footer}
        </div>
    ...
    <div className="flex h-full min-h-0 overflow-hidden bg-primary-50/95 dark:bg-neutral-950">
      ...
      >
        <DialogContent>
          <div className="p-5 space-y-3">
            <DialogTitle>
              {promptState?.mode === 'rename' ? '이름 변경' : '새 폴더'}
            </DialogTitle>
            <DialogDescription>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={() => void handlePromptSubmit()}>저장</Button>
            </div>
          </div>
        </DialogContent>
      </DialogRoot>
      ...
      >
        <DialogContent>
          <div className="p-5 space-y-3">
            <DialogTitle>
              {deleteConfirm?.type === 'folder' ? '폴더 삭제' : '파일 삭제'}
            </DialogTitle>
            <DialogDescription>
              정말 삭제하시겠습니까{' '}
